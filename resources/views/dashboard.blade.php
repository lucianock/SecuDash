<x-layouts.app>
  <div class="p-6 bg-neutral-900 min-h-screen">
    ...
    <div class="grid gap-6 md:grid-cols-4 mb-8">
      <!-- Uptime -->
      <div class="bg-neutral-800 rounded-2xl shadow-lg p-5 flex items-center space-x-4 hover:scale-105 transition-transform">
        ...
        <div class="grid grid-cols-2 gap-4">
          <div>
            ...
            <table class="min-w-full text-sm text-left text-white">
              <thead><tr><th class="py-1">PID</th><th>%CPU</th></tr></thead>
          ...
          <div>
            ...
            <table class="min-w-full text-sm text-left text-white">
              <thead><tr><th class="py-1">PID</th><th>%MEM</th></tr></thead>
  ...
  <script>
    const { CountUp } = countUp; // UMD exposes window.countUp
    ...
    async function cargarMetricas() {
      ...
      new CountUp('cardProc', d.processes.count, { duration: 1 }).start();       // number
      new CountUp('cardServices', Object.values(d.services).filter(s => s==='active').length, { duration: 1 }).start();
      ...
      new Chart(document.getElementById('memChart'), {
        ...
        data: {
          labels: ['Usada','Libre'],
          datasets: [{ data: [d.memory.used_percent,100-d.memory.used_percent], backgroundColor: ['#3b82f6','#374151'] }]
        },
      ...
      new Chart(document.getElementById('diskChart'), {
        ...
        data: {
          labels: ['Usado','Libre'],
          datasets: [{ data: [d.disk.used_percent,100-d.disk.used_percent], backgroundColor: ['#ef4444','#374151'] }]
        },
        options: { responsive: true, plugins:{ legend:{ labels:{ color:'#ddd' } } } }
      });
      ...
      new Chart(document.getElementById('netChart'), {
        ...
        data: {
          labels: labelsNet,
          datasets: [
            { label:'RX', data: rx, backgroundColor: '#10b981' },
            { label:'TX', data: tx, backgroundColor: '#f59e0b' }
          ]
        },
      ...
      d.processes.top_cpu.trim().split('\n').slice(1).forEach(l => {
        ...
        cpuRows.innerHTML += `<tr class="hover:bg-neutral-700"><td class="py-1">${pid}</td><td>${pc}</td></tr>`;
      });
      const memRows = document.getElementById('tableMemRows');
      memRows.innerHTML = '';
      d.processes.top_mem.trim().split('\n').slice(1).forEach(l => {
        const [pid,,pm] = l.trim().split(/\s+/);
        memRows.innerHTML += `<tr class="hover:bg-neutral-700"><td class="py-1">${pid}</td><td>${pm}</td></tr>`;
      });
    ...
    cargarMetricas();
    setInterval(()=>{ if(autoRefresh) cargarMetricas(); }, 5000);
  </script>
</x-layouts.app>
